...
"""
function CI.Model(sys::ContinuousResourceSharer; states::AbstractVector{Symbol}, parameters, invariants=AbstractInvariant[], kw...)
    length(states) == nstates(sys) || error("expected $(nstates(sys)) state names")
    Model(sys; parameters, states=Dict(s => i for (i, s) in enumerate(states)), kind=:continuous, invariants, kw...,
          metadata=Dict{Symbol,Any}(:source => :AlgebraicDynamics))
end
function CI.Model(sys::DiscreteResourceSharer; states::AbstractVector{Symbol}, parameters, invariants=AbstractInvariant[], kw...)
    length(states) == nstates(sys) || error("expected $(nstates(sys)) state names")
    Model(sys; parameters, states=Dict(s => i for (i, s) in enumerate(states)), kind=:discrete, invariants, kw...,
          metadata=Dict{Symbol,Any}(:source => :AlgebraicDynamics))
end
